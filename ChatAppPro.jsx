...
import { 
    ...
    serverTimestamp, getDocs, setDoc, deleteDoc, updateDoc
} from 'firebase/firestore';

// --- Environment Variables (NOW HARDCODED WITH YOUR CONFIG) ---
// Note: We use the projectId as the appId for Firestore path segmentation.
...
// Placeholder for __initial_auth_token since it's not provided
...
const App = () => {
  ...
  const [userRoles, setUserRoles] = useState({}); // { userId: { role: 'owner', isBanned: false, username: '...' } }
  ...
  const isBanned = currentUserRole.isBanned;

  // Chat/Navigation State
  const [activeTab, setActiveTab] = useState('public'); // 'public', 'private', 'admin'
  const [activeChatRecipient, setActiveChatRecipient] = useState(null); // Full user object for PM target
  ...
  const toggleBanUser = useCallback(async (targetUid, isCurrentlyBanned) => {
    if (!isOwner || !db || targetUid === user.uid) return; // Cannot ban self
  ...
  const toggleModRole = useCallback(async (targetUid, currentRole) => {
    if (!isOwner || !db || targetUid === user.uid) return; // Cannot change own role
                    ...
                    <div key={u.uid} className={`flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 rounded-xl shadow-lg transition-all duration-300 ${u.isBanned ? 'bg-red-100 dark:bg-red-900/50' : u.role === 'mod' ? 'bg-yellow-50 dark:bg-yellow-900/50' : 'bg-white dark:bg-gray-700/80'}`}>
                        <div className="mb-2 sm:mb-0">
                            ...
                            <div className="flex items-center space-x-2 mt-1">
                                {u.role === 'owner' && <span className="flex items-center text-indigo-600 dark:text-indigo-400 text-sm font-medium"><CrownIcon className="mr-1" /> OWNER</span>}
                                {u.role === 'mod' && <span className="flex items-center text-yellow-600 dark:text-yellow-400 text-sm font-medium"><ShieldIcon className="mr-1" /> MODERATOR</span>}
  ...
  const ChatContent = ({ chatType, recipient }) => {
    ...
    const handleSendMessage = useCallback(async () => {
        if (!db || !user || newMessageText.trim() === '' || isBanned) return;
    ...
    }, [db, user, newMessageText, chatPath, isBanned]);

    const deleteMessage = useCallback(async (messageId) => {
        if (!db || !isMod) return; // Only Mods/Owner can delete in public chat
        ...
        return (
            <div className={`flex mb-4 ${isCurrentUser ? 'justify-end' : 'justify-start'}`}>
                <div className={`flex items-start max-w-xs sm:max-w-md lg:max-w-lg ${isCurrentUser ? 'flex-row-reverse' : 'flex-row'} space-x-2 ${!isCurrentUser ? 'sm:space-x-reverse' : ''}`}>
                    ...
                    {/* Delete button (Owner/Mod only in public chat) */}
                    {isMod && chatType === 'public' && !isCurrentUser && (
                        <button 
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 font-[Inter] transition-colors duration-500">
